class Visualiser {
    ...
    init () {
        const number = Math.floor(this.data.reduce((a, b) => a + b) / this.data.length) / 10
        this.p5.clear()
        ...
        this.randomx = this.p5.random(number)
        this.randomy = this.p5.random(number)


        this.randomX2 = this.p5.random(this.width / 2.2)
        this.randomY2 = this.p5.random(this.height / 2.2)
        this.randomx2 = this.p5.random(number)
        this.randomy2 = this.p5.random(number)
        this.modifier = 0
        this.paused   = false
    }
    ...
    show ( dataArray ) {
        ...
        for ( let i = 0; i < this.speed; i++ ) {
            let startX = this.randomX * this.p5.cos(this.modifier) + this.width / 2
